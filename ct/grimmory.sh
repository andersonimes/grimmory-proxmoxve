...
# Originally authored as a BookLore script by community-scripts; adapted for Grimmory by dalenjohnson
# (https://gist.github.com/databoy2k/1ef8c9ccf93d94d6ac776fa1befb182e). Forked here by andersonimes.
...
function update_script() {
  ...
  if check_for_gh_release "grimmory" "grimmory-tools/grimmory"; then
    ...
    if ! systemctl is-active --quiet grimmory.service; then
      msg_error "Grimmory service failed to start. Backups were retained at /opt/booklore_bak or /opt/grimmory_bak."
      exit 1
    fi
